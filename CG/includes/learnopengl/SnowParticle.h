...
#define MIN_SIZE 5.0f
	...
	const int MAX_PARTICLES = 1500;//定义粒子发射系统最大的粒子数

								   //初始发射器例子数量
	const int INIT_PARTICLES = 800;
	...
	public:
		SnowParticle()
		{
			...
			mUpdateShader = new Shader(FileSystem::getPath("shaders/SnowUpdate.vs").c_str(), FileSystem::getPath("shaders/SnowUpdate.fs").c_str(),
				FileSystem::getPath("shaders/SnowUpdate.gs").c_str(), varyings, 5);
			//设置TransformFeedback缓存能够记录的顶点的数据类型

			mRenderShader = new Shader(FileSystem::getPath("shaders/SnowRender.vs").c_str(), FileSystem::getPath("shaders/SnowRender.fs").c_str());
			//设置随机纹理
			InitRandomTexture(128);
			mSparkTexture.loadTexture(FileSystem::getPath("res/rock/rock.png"));
			mRenderShader->use();
