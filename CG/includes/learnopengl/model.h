...
public:
	...
	void info() {
		for (unsigned int i = 0; i < meshes.size(); i++) {
			vector<ATexture> tex = meshes[i].textures;
			for (unsigned int j = 0; j < tex.size(); j++) {
				cout << tex[j].id << " " << tex[j].type << '\n';
			}
		}
	}
			...
			if (strcmp(nodepairs[nodepairsindx].first.name, nodename) == 0)
			{
				...
				for (int animationchannelindex = 0; animationchannelindex < animations[0].numChannels; animationchannelindex++)
				{
					if (strcmp(animations[0].channels[animationchannelindex].nodeName, nodename) == 0)
					{
						...
						glm::mat4 rotationM = glm::mat4(rotationkey.keyData);
						glm::mat4 scalingM = glm::scale(scalingkey.keyData);
						glm::mat4 translateM = glm::translate(positionKey.keyData);
